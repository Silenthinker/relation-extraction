...
def evaluate(trainer, data_loader, t_map, cuda=False):
    ...
    for sample in chain.from_iterable(data_loader):
        ...
        output = trainer.valid_step(sample)
...
def main():
    ...
    if os.path.isfile(args.load_checkpoint):
        dev_prec, dev_rec, dev_f1 = evaluate(trainer, val_loader, target_map, cuda=args.cuda)
        test_prec, test_rec, test_f1 = evaluate(trainer, test_loader, target_map, cuda=args.cuda)
